 ...
 */
public class FireMaster extends GameAPI {
    ...
    public void onEnable() {
       ...
       setNeedsMapRestore(true);
        setAllowBuilding(false);
        this.getServer().getPluginManager().registerEvents(new InstanceEvents(this), this);
       // setAttackListener(new AttackListener());

        this.getAttackListener().start();

        loadInstances();
        System.out.print(getGameInstanceManager().getGameInstances().size() + " ARENAS LOADED!");
        loadLanguageFile();
