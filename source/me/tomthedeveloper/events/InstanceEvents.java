...
public class InstanceEvents implements Listener {
	...
	public void onFireBlast(PlayerInteractEvent event){
		if(!(event.getAction() == Action.LEFT_CLICK_AIR || event.getAction() == Action.LEFT_CLICK_BLOCK))
			return;
		if(FireMaster.instance.getGameAPI().getGameInstanceManager().getGameInstance(event.getPlayer()) == null)
			return;
		if(event.getPlayer().isSneaking())
			return;
		GameInstance gameInstance = FireMaster.instance.getGameAPI().getGameInstanceManager().getGameInstance(event.getPlayer());
	...
	public void onPlayerDamage(EntityDamageEvent event) {
		...
		if(!(ent instanceof Player))
			return;
		if(FireMaster.instance.getGameAPI().getGameInstanceManager().getGameInstance((Player) ent) == null)
			return;
	...
	public void onLightningStrike(PlayerInteractEvent event){
		...
		if(event.getPlayer().getTargetBlock((HashSet<Byte>) null, 50) == null) {
			event.getPlayer().sendMessage(gameInstance.getChatManager().getMessage("UnableToStrikeThereMessage"));
			return;
		}
		if( event.getPlayer().getTargetBlock((HashSet<Byte>) null, 50).getType() == Material.AIR){
			event.getPlayer().sendMessage(gameInstance.getChatManager().getMessage("UnableToStrikeThereMessage"));
			return;
		}
	...
	public void onPlayerDie(final PlayerDeathEvent event) {
		...
		if (FireMaster.instance.getGameAPI().getGameInstanceManager().getGameInstance(p) == null)
			return;



		final GameInstance gameInstance = FireMaster.instance.getGameAPI().getGameInstanceManager().getGameInstance(p);
	...
	public void onEntityExplode(BlockExplodeEvent e){
		...
		for(Player player: e.getBlock().getLocation().getWorld().getPlayers()){
			if(FireMaster.instance.getGameAPI().getGameInstanceManager().getGameInstance(player) != null){
				boo = true;
				break;
			}
		}
	...
	public void onDamage(EntityDamageEvent event){
		if(!(event.getEntity() instanceof Player))
			return;
		if((FireMaster.instance.getGameAPI().getGameInstanceManager().getGameInstance((Player) event.getEntity())) == null)
			return;
		GameInstance gameInstance = FireMaster.instance.getGameAPI().getGameInstanceManager().getGameInstance((Player) event.getEntity());
		if(gameInstance.getGameState() == GameState.STARTING || gameInstance.getGameState() == GameState.WAITING_FOR_PLAYERS)
			event.setCancelled(true);
	}
	...
	public void onOpenMenu(final PlayerInteractEvent event){
		...
		if(FireMaster.instance.getGameAPI().getGameInstanceManager().getGameInstance(event.getPlayer()) == null)
			return;
		final GameInstance gameInstance = FireMaster.instance.getGameAPI().getGameInstanceManager().getGameInstance(event.getPlayer());
		SpectatorMenu spectatorMenu = new SpectatorMenu(FireMaster.instance.getGameAPI().getGameInstanceManager().getGameInstance(event.getPlayer())) {
			@Override
	...
	public void onSpectatorOptionClick(InventoryClickEvent event){
		...
		if(FireMaster.instance.getGameAPI().getGameInstanceManager().getGameInstance(whoclicked) == null)
			return;
		GameInstance gameInstance;
		if(FireMaster.instance.getGameAPI().isBungeeActivated()){
			gameInstance = FireMaster.instance.getGameAPI().getGameInstanceManager().getGameInstances().get(0);
		}else {
			gameInstance = FireMaster.instance.getGameAPI().getGameInstanceManager().getGameInstance(whoclicked);
		}
	...
	public void onAddSpawn(PlayerAddSpawnCommandEvent event){
		...
		FireMaster.instance.getGameAPI().saveLoc("instances."+ event.getArenaID() + ".spawnlocations." + event.getSpawnName(), event.getPlayer().getLocation());
		plugin.saveConfig();
